<!doctype html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport"
          content="width=device-width, user-scalable=no, initial-scale=1.0, maximum-scale=1.0, minimum-scale=1.0">
    <meta http-equiv="X-UA-Compatible" content="ie=edge">
    <link rel="icon" type="image/x-icon" href="../imgs/inlFavicon@4x.png">
    <link rel="stylesheet" href="client-nav-style.css">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.6.0/css/all.min.css"/>
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.7.0/css/font-awesome.min.css">


    <title>Navigation Template</title>
</head>
<body>
<div class="topvar">
    <div class="logo">
        <img src="../imgs/inl2Logo.png" alt="">
    </div>
    <div class="top-right">
        <div class="right-btn">
            <button class="btn-top"><a href="../loginSignup/index.php"><i class="fa-solid fa-magnifying-glass"></i> Browse</a></button>
            <button class="btn-top"><a href="../loginSignup/logIn.php"><i class="fa-solid fa-right-from-bracket"></i> Logout</a></button>
        </div>
        <div class="profile">
            <a href="client-profile-page.php"><img src="../imgs/profile.png" alt=""></a>
        </div>
        <div class="name">
            <a href="client-profile-page.php"><h4 style="font-weight: 700">Natasha</h4></a>
            <p style="font-size: 12px">Client</p>
        </div>
    </div>

</div>
    <div class="navbar">
        <div class="sidebar-frame">
            <ul class="side-content">
                <li><a href="client-dashboard-page.php"><i class="fa-solid fa-database"></i> Dashboard</a></li>
                <li><a href="client-project-page.php"><i class="fa-solid fa-chart-simple"></i> Projects</a></li>
                <li><a href="client-payment-page.php"><i class="fa-solid fa-dollar-sign"></i> Paymants</a></li>
                <li><a href="client-notification-page.php"><i class="fa-solid fa-bell"></i> Notification</a></li>
                <li><a href="client-message-page.php"><i class="fa-solid fa-envelope"></i> Message</a></li>
                <li><a href="client-profile-page.php"><i class="fa-solid fa-circle-user"></i> Profile</a></li>
            </ul>
            <div class="help">
                <h4><a href="#"><i class="fa-solid fa-circle-info"></i> Help & Support</a></h4>
            </div>

        </div>
    </div>
</body>
</html>
